...
import Multitier.Debugger.TimeLine as TimeLine exposing (TimeLine, Event(..), ServerEventType(..), ServerMsgType(..), ClientEventType(..))
...
wrapInitServer (serverModel, cmd) =
  ...
  { debugger =
    { state = Running
    , appModel = serverModel
    , timeline = TimeLine.empty (serverModel, cmd) |> TimeLine.pushServerEvent (InitServer msgCount, serverModel, cmd)
    ...
    , rpcMsgCount = 0 }
  , clients =
    { debugging = Dict.empty }} ! [Cmd.map (ServerAppMsg None) cmd]
          ...
          let (newAppModel, cmd) = updateServer serverAppMsg debugger.appModel in
            case parent of
              ...
              RegularServerMsg parentID ->
                { serverModel | debugger = { debugger | appModel = newAppModel, msgCount = debugger.msgCount + 1
                , timeline = TimeLine.pushServerEvent (((ServerMsgEvent (ChildServerMsg parentID debugger.msgCount)) serverAppMsg), newAppModel, cmd) debugger.timeline }}
                  ! [Cmd.map (ServerAppMsg (RegularServerMsg debugger.msgCount)) cmd]
              ServerRPC cid rpcid ->
...
wrapServerRPCs serverRPCs = \remoteServerMsg -> case remoteServerMsg of
  ...
  RemoteServerAppMsg cid rpcid msg ->
    RPC.map (AppMsg (Just rpcid)) (ServerAppMsg (ServerRPC cid rpcid))
      (\appModel serverModel ->
        let debugger = serverModel.debugger in
          case debugger.state of
            Running ->
...
serverEventView event =
  case event of
    InitServer msgid -> "[Init-Server]"
    ServerMsgEvent msgtype msg -> case msgtype of
      NewServerMsg msgid -> "[NewServerMsg](" ++ (toString msgid) ++ ")" ++ (toString msg)
      ChildServerMsg parentid msgid -> "[ChildServerMsg](" ++ (toString parentid) ++ "," ++ (toString msgid) ++ ")" ++ (toString msg)
